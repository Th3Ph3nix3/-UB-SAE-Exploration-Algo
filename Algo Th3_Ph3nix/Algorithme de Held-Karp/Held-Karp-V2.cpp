...
int main(int argc, char* argv[]) {
    ...
    auto duree = std::chrono::duration_cast<std::chrono::microseconds>(end - start);

    std::cout << "Cout minimum TSP : " << result << std::endl;
    std::cout << "Temps d'exécution : " << duree.count() << " microsecondes." << std::endl;

    // Write timing data
    std::ofstream times("temps_execution.txt");
    if (times.is_open()) {
        times << duree.count() << std::endl;
        times.close();
    } else {
